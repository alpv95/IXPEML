...
class NetTest(object):
    ...
    def __init__(self,
        ...
        self.datasets = [
            os.path.join(self.data_base, data) for data in datasets
        ]
    ...
    def _predict(self,
                 net,
                 dataset,
                 augment=3):  #base single net on single dataset prediction
        '''
        ...
        dataset = H5DatasetEval(dataset,
                                datatype=datatype,
                                losstype=self.losstype,
                                energy_cal=(meanE, stdE),
                                augment=augment,
                                transform=transforms.Compose(
                                    [ZNormalize(mean=mean, std=std)]))
        ...
        mom_abs_pts = np.reshape((dataset.mom_abs_pts).numpy(), (-1, 2),
                                 order="C")
        p_tail = [None]
        ...
        if datatype == 'sim':
            ...
            xy_abs_pts_true = dataset.xy_abs_pts_true.numpy()
        ...
        if energies_sim[0] is not None:
            energies_sim = np.round(energies_sim * stdE.item() + meanE.item(),
                                    3)
        energies_mom = (dataset.mom_energy).numpy()

        return angles, angles_mom, angles_sim, moms, errors, abs_pts, mom_abs_pts, abs_pts_sim, energies, energies_sim, energies_mom, zs, trgs, flags, p_tail, xy_abs_pts, xy_abs_pts_true
    ...
    def stokes_correction(self, angles):
        ...
        with fits.open(self.base + "real_data/systematic_UnpFF_DU2.fits",
                       memmap=False) as hdu:
            Q = hdu[1].data
            U = hdu[2].data
            ENE = hdu[3].data
        energy_idx = np.argmin(
            abs(ENE - self.stokes_correct))  #which energy map to use
        ...
        for i, angle in enumerate(angles[:2]):
            ...
            Q_final = Q_init + (Q[energy_idx, x_grid, y_grid] +
                                Q[energy_idx, x_grid + 1, y_grid] +
                                Q[energy_idx, x_grid, y_grid + 1] +
                                Q[energy_idx, x_grid + 1, y_grid + 1] +
                                Q[energy_idx, x_grid + 1, y_grid - 1] +
                                Q[energy_idx, x_grid - 1, y_grid + 1] +
                                Q[energy_idx, x_grid - 1, y_grid] +
                                Q[energy_idx, x_grid, y_grid - 1] +
                                Q[energy_idx, x_grid - 1, y_grid - 1]) / 9
            U_final = U_init + (U[energy_idx, x_grid, y_grid] +
                                U[energy_idx, x_grid + 1, y_grid] +
                                U[energy_idx, x_grid, y_grid + 1] +
                                U[energy_idx, x_grid + 1, y_grid + 1] +
                                U[energy_idx, x_grid + 1, y_grid - 1] +
                                U[energy_idx, x_grid - 1, y_grid + 1] +
                                U[energy_idx, x_grid - 1, y_grid] +
                                U[energy_idx, x_grid, y_grid - 1] +
                                U[energy_idx, x_grid - 1, y_grid - 1]) / 9
        ...
        for data in self.datasets:
            ...
            for i, net in enumerate(self.nets[:3]):
                print(">> Angular NN {}/{} : \n".format(
                    i + 1, len(self.nets[:3])))
                results = tuple(
                    map(np.append, results, self._predict(net, data,
                                                          augment=6)))
                print(">> Complete")
            ...
            for i, net in enumerate(self.nets[3:]):
                print(">> Ptail NN {}/{} : \n".format(i + 1,
                                                      len(self.nets[3:])))
                results_ptail = tuple(
                    map(np.append, results_ptail,
                        self._predict(net, data, augment=3)))
                print(">> Complete")

            p_tail = triple_angle_reshape(results_ptail[-3],
                                          len(self.nets[3:]),
                                          augment=3)
            ...
            if self.save_table is not None:
                name = data.replace(
                    self.data_base,
                    "") + "__" + self.save_table + "__" + "ensemble"
                fits_save(results, p_tail,
                          os.path.join(self.save_base, name.replace("/", "_")),
                          self.datatype, self.losstype)
                print("Saved to: {}".format(
                    os.path.join(self.save_base,
                                 name.replace("/", "_") + ".fits")))
